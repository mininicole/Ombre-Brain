...
# =============================================================
# /api/admin/audit_tg_domain — 一次性迁移辅助：统计需要补 tg-private 的桶
# GET 返回 stats + 候选 ID 列表（dry-run，不改动）
# POST 带 {"confirm":true,"domain_to_add":"tg-private"} 才真正写入
# =============================================================
@mcp.custom_route("/api/admin/audit_tg_domain", methods=["GET", "POST"])
async def api_admin_audit_tg_domain(request):
    from starlette.responses import JSONResponse
    try:
        all_buckets = await bucket_mgr.list_all(include_archive=False)
    except Exception as e:
    ...
    for b in all_buckets:
        ...
        if isinstance(doms, str):
            doms = [doms]
        if "tg-gale" in doms:
            already_tg_gale.append(b["id"])
        elif "tg-private" in doms:
            already_tg_private.append(b["id"])
        else:
            needs_tg_private.append({
                "id": b["id"],
                "name": meta.get("name", ""),
                "domain": doms,
    ...
    stats = {
        ...
        "already_tg_gale": len(already_tg_gale),
        "already_tg_private": len(already_tg_private),
        "needs_tg_private": len(needs_tg_private),
    }

    if request.method == "GET":
        return JSONResponse({"mode": "dry-run", "stats": stats, "candidates_preview": needs_tg_private[:20], "total_candidates": len(needs_tg_private)})
    ...
    if not body.get("confirm"):
        return JSONResponse({"error": "POST 需带 {\"confirm\": true} 才执行写入"}, status_code=400)
    tag = str(body.get("domain_to_add") or "tg-private").strip()
    if not tag:
        return JSONResponse({"error": "domain_to_add 不能为空"}, status_code=400)

    updated = 0
    failed = []
    for cand in needs_tg_private:
        bid = cand["id"]
        try:
            current_doms = list(cand["domain"])
            if tag not in current_doms:
                current_doms.append(tag)
            await bucket_mgr.update(bid, domain=current_doms)
            updated += 1
        except Exception as e:
            failed.append({"id": bid, "error": str(e)})
    return JSONResponse({"mode": "applied", "updated": updated, "failed": failed, "stats": stats})
